...
| AD-018 | Error handling is inconsistent across controllers: HTTP status codes for similar failures vary, message/shape drift exists, and historical logging has been mostly ad-hoc console output. A centralized error stack (error classes + middleware + structured logger) is now introduced and partially applied, but full controller-surface migration is still pending. | Medium observability and ops risk; production failures are difficult to diagnose; inconsistent client-facing error shapes complicate UI error handling. | Medium | In Progress | Complete migration of all controllers to centralized error classes/middleware and standard response codes/shapes. Preserve policy-safe messages while recording structured error context in logs. |
