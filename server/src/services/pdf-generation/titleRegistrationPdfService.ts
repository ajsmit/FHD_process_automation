...
export async function renderTitleRegistrationPdfDocument(
  outFile: string,
  formData: FormData,
  options: {
    repoRoot: string;
    coSupervisor1Name: string;
    coSupervisor2Name: string;
  },
): Promise<void> {
  ...
  page.drawText('Higher Degree Committee', { x: 130, y: headerTopY - 8, size: 16, font: fontBold, color: rgb(0.05, 0.11, 0.2) });
  page.drawText('Title registration and Appointment of Supervisor', { x: 130, y: headerTopY - 30, size: 12, font: fontBold, color: rgb(0.08, 0.08, 0.08) });
  const dividerY = headerTopY - 42;
  page.drawLine({ start: { x: 36, y: dividerY }, end: { x: pageW - 36, y: dividerY }, thickness: 0.8, color: rgb(0.7, 0.74, 0.8) });
  ...
  const drawField = (label: string, value: string, height = rowH) => {
    const labelLines = wrapTextLines(label, { maxWidth: labelW - 12, size: 8.8, font: fontBold, maxLines: 3 });
    const valueLines = wrapTextLines(value || '-', { maxWidth: valueW - 12, size: 9.2, font: fontRegular, maxLines: 8 });
    ...
    page.drawRectangle({ x: margin, y: y - effectiveHeight, width: labelW, height: effectiveHeight, borderWidth: 0.7, borderColor: rgb(0.8, 0.84, 0.88) });
    page.drawRectangle({ x: margin + labelW, y: y - effectiveHeight, width: valueW, height: effectiveHeight, borderWidth: 0.7, borderColor: rgb(0.8, 0.84, 0.88) });
  ...
  drawField(
    ...
    `${formData['Administrative Supervisor same as Supervisor'] === 'Yes' ? '[Same as Supervisor] ' : ''}${formData['Administrative Supervisor (Nominal Role)']} (${formData['Administrative Supervisor Qualifications (Nominal Role)']})`,
  );
  drawField('Has Co-supervisor?', formData['Has Co-supervisor?']);
  if (formData['Has Co-supervisor?'] === 'Yes') {
    drawField(
      'Co-supervisor 1',
      formData['Co-supervisor is UWC-internal'] === 'Yes'
    ...
    if (hasNamedPerson(options.coSupervisor2Name)) {
      drawField(
        'Co-supervisor 2',
        formData['Second Co-supervisor is UWC-internal'] === 'Yes'
  ...
  drawField(
    'Ethics clearance',
    `${formData['Does this project need Ethics clearance?']}${formData['Ethics clearance reference number'] ? ` | Ref: ${formData['Ethics clearance reference number']}` : ''}${formData['Date on which ethics clearance was issued'] ? ` | Date: ${formData['Date on which ethics clearance was issued']}` : ''}`,
    30,
  );
  ...
  page.drawRectangle({ x: margin, y: y - 135, width: pageW - margin * 2, height: 135, borderWidth: 0.9, borderColor: rgb(0.7, 0.74, 0.8) });
  page.drawRectangle({ x: margin, y: y - 20, width: pageW - margin * 2, height: 20, color: rgb(0.94, 0.96, 0.98) });
  page.drawText('Abstract (maximum 200 words)', { x: margin + 6, y: y - 14, size: 9.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  drawWrappedText(page, formData.Abstract || '-', {
    x: margin + 8,
    y: y - 34,
  ...
  page.drawText('Signatures', { x: margin, y: y - 3, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  y -= 16;
  ...
  for (const role of sigRows) {
    page.drawRectangle({ x: sigCols[0], y: y - 18, width: 180, height: 18, borderWidth: 0.7, borderColor: rgb(0.8, 0.84, 0.88) });
    page.drawRectangle({ x: sigCols[1], y: y - 18, width: 180, height: 18, borderWidth: 0.7, borderColor: rgb(0.8, 0.84, 0.88) });
    page.drawRectangle({ x: sigCols[2], y: y - 18, width: 100, height: 18, borderWidth: 0.7, borderColor: rgb(0.8, 0.84, 0.88) });
    page.drawText(role, { x: sigCols[0] + 6, y: y - 12, size: 8.8, font: fontRegular, color: rgb(0.2, 0.2, 0.2) });
    y -= 18;
  }
