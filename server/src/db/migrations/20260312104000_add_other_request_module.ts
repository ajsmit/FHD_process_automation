...
export async function up(knex: Knex): Promise<void> {
  if (!(await knex.schema.hasTable('other_request_forms'))) {
    await knex.schema.createTable('other_request_forms', (table) => {
...
export async function down(knex: Knex): Promise<void> {
  if (await knex.schema.hasTable('other_request_forms')) {
    await knex.schema.dropTable('other_request_forms');
  }
}
