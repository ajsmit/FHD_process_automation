...
export async function up(knex: Knex): Promise<void> {
  if (!(await knex.schema.hasTable('upgrade_msc_to_phd_forms'))) {
    await knex.schema.createTable('upgrade_msc_to_phd_forms', (table) => {
...
export async function down(knex: Knex): Promise<void> {
  if (await knex.schema.hasTable('upgrade_msc_to_phd_forms')) {
    await knex.schema.dropTable('upgrade_msc_to_phd_forms');
  }
}
