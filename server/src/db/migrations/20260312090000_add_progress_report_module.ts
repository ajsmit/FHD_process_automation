...
export async function up(knex: Knex): Promise<void> {
  if (!(await knex.schema.hasTable('progress_report_forms'))) {
    await knex.schema.createTable('progress_report_forms', (table) => {
...
export async function down(knex: Knex): Promise<void> {
  if (await knex.schema.hasTable('progress_report_forms')) {
    await knex.schema.dropTable('progress_report_forms');
  }
}
